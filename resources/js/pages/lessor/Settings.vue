<template>
  <div class="max-w-lg mx-auto bg-white rounded-xl shadow-lg p-8">
    ...
    <form @submit.prevent="save" class="flex flex-col gap-4">
      ...
      <hr class="my-2" />
      <p class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Company GCash</p>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">GCash Number</label>
        <input v-model="form.gcash_number" type="text" placeholder="09XXXXXXXXX" class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400" />
      </div>
...
const form = ref({ name: '', email: '', password: '', password_confirmation: '', gcash_number: '' })
const busy = ref(false)
const successMsg = ref('')
const errorMsg = ref('')

onMounted(async () => {
  const { data } = await axios.get('/lessor/api/settings')
  form.value.name         = data.user.name
  form.value.email        = data.user.email
form.value.gcash_number = data.user.gcash_number ?? ''
})
...
const save = async () => {
  ...
  errorMsg.value = ''
  try {
    const { data } = await axios.put('/lessor/api/settings', form.value)
